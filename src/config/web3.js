...
export const joinPool = async (
    ...
    if (poolAddr) {
        ...
        const inMaxAmount = await toWeiVal(provider, tokenA, (amountA * 1.2));
        const outAmount = await toWeiVal(provider, tokenB, amountB);
        const outMaxAmount = await toWeiVal(provider, tokenB, amountB * 1.2);
...
export const removePool = async (
    ...
    const tokenRatio = await toWeiVal(provider, token2Addr, ratio);
...
const toWeiVal = async (provider, tokenAddr, val) => {
    ...
    if (decimal === 21)
        return web3.utils.toWei(val.toFixed(6), 'kether');
    else if (decimal === 18)
        return web3.utils.toWei(val.toFixed(6), 'ether');
    else if (decimal === 15)
        return web3.utils.toWei(val.toFixed(6), 'milliether');
    else if (decimal === 12)
        return web3.utils.toWei(val.toFixed(6), 'microether');
    else if (decimal === 9)
        return web3.utils.toWei(val.toFixed(6), 'nano');
    else
        return web3.utils.toWei(val.toFixed(6));
}
