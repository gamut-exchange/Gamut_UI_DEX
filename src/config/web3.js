...
export const getHoldingInLP = async (provider, account, poolList, factoryContract) => {
    let web3 = new Web3(provider);
    const tokenAbi = erc20ABI;
    const pAbi = poolABI;
    let tvlBalance = 0;
    ...
    for (let i = 0; i < poolList.length; i++) {
        try {
            ...
            if (Number(userlp) !== 0) {
                ...
                let price1 = 0;
                let price2 = 0;
                if (poolTokenAndBalance?.tokens[0].toLowerCase() !== "0xeb466342c4d449bc9f53a865d5cb90586f405215".toLowerCase()) {
                    let poolAddress1 = await getPoolAddress(provider, poolTokenAndBalance?.tokens[0], "0xeb466342c4d449bc9f53a865d5cb90586f405215", factoryContract);
                    let poolData1 = await getPoolData(provider, poolAddress1);
                    price1 = await calculateSwap(poolTokenAndBalance?.tokens[0], poolData1, 1);
                } else {
                    price1 = 1;
                }

                if (poolTokenAndBalance?.tokens[1].toLowerCase() !== "0xeb466342c4d449bc9f53a865d5cb90586f405215".toLowerCase()) {
                    let poolAddress2 = await getPoolAddress(provider, poolTokenAndBalance?.tokens[1], "0xeb466342c4d449bc9f53a865d5cb90586f405215", factoryContract);
                    let poolData2 = await getPoolData(provider, poolAddress2);
                    price2 = await calculateSwap(poolTokenAndBalance?.tokens[1], poolData2, 1);
                } else {
                    price2 = 1;
                }
                let lp = (price1 * (poolTokenAndBalance?.balances[0] / 10 ** decimal1) +
                    price2 * (poolTokenAndBalance?.balances[1] / 10 ** decimal2)) / (supply / 10 ** 18);
                LPHolding.push({
                    ...poolList[i],
                    address: poolAddress,
                    apr: apr,
...
export const getHoldingInFarms = async (provider, account, farmList, factoryContract) => {
    let web3 = new Web3(provider);
    const tokenAbi = erc20ABI;
    const pAbi = poolABI;
...
export const getAllFarmPools = async (provider, account, farmList, factoryContract) => {
    let web3 = new Web3(provider);
    const tokenAbi = erc20ABI;
    const pAbi = poolABI;
    const farmAbi = farmABI;
    ...
    for (let i = 0; i < farmList.length; i++) {
        ...
        if (Number(totallp) !== 0) {
            ...
            let lp = (price1 * (poolTokenAndBalance?.balances[0] / 10 ** decimal1) +
                price2 * (poolTokenAndBalance?.balances[1] / 10 ** decimal2)) / (supply / 10 ** 18);
            let apr2 = calcStakingAPR(rewardPerBlock, startBlock, endBlock, lp * userstakedlp);
            pools.push({
                ...farmList[i],
                apr: (apr1 * 1 + apr2 * 1),
                userlp: userlp * 1,
...
const calcStakingAPR = (rewardPerBlock, startBlock, endBlock, stakedlpusd) => {
    if (stakedlpusd !== 0) {
        ...
        let apr = ((rewardusd / stakedlpusd) / (60 * 365)) * 100;
        return apr;
    } else {
        return 0;
    }
...
export const calculateSwap = async (inToken, poolData, input) => {
    ...
    let amount = input * 10 ** decimal_from;

    let bIn = amount / 10 ** decimal_from;
...
export const getUserNfts = async (provider, groupList, contractAddr, account) => {
    let web3 = new Web3(provider);
...
export const getAllStakingPools = async (provider, token_id, boostToEpochList, contractAddr) => {
    let web3 = new Web3(provider);
    let nftContract = new web3.eth.Contract(nftABI, contractAddr);
    let result = [];
    let pool_ids = [];
    boostToEpochList.map((item) => {
        if (pool_ids.indexOf(item.pool_id) === -1)
            pool_ids.push(item.pool_id);
    });

    for (let i = 0; i < pool_ids.length; i++) {
        let item = await nftContract.methods['stakingPools'](pool_ids[i]).call();
        item.isStaked = false;
        item.nftId = 0;
        for (let j = 0; j < boostToEpochList.length; j++) {
            if (boostToEpochList[j].pool_id === pool_ids[i]) {
                var boostPoolInfo = await nftContract.methods['getPoolInfo'](token_id, boostToEpochList[j].id).call();
                if (Number(boostPoolInfo.amount) !== 0) {
                    item.amount = boostPoolInfo.amount;
                    item.isStaked = true;
                    item.nftId = token_id;
                    break;
                } else {
        ...
        result.push({
            address: item.stakingToken, amount: web3.utils.fromWei(item.amount.toString()), poolId: item.poolId, groupMultiplier: item.groupMultiplier,
            timeMultiplier: item.timeMultiplier, timeBoostLength: item.timeBoostLength, isStaked: item.isStaked, nftId: item.nftId
        });
    }
    return result;
}

export const getPoolEpochs = async (provider, boostToEpochList, nftId, poolId, contractAddr) => {
    let web3 = new Web3(provider);
    let nftContract = new web3.eth.Contract(nftABI, contractAddr);
    let result = [];
...
export const mintNft = async (provider, groupName, startTokenId, lastTokenId, mintPrice, tokenAddr, contractAddr, account) => {
    let web3 = new Web3(provider);
    let tokenContract = new web3.eth.Contract(erc20ABI, tokenAddr);
    let nftContract = new web3.eth.Contract(nftABI, contractAddr);
    for (let i = startTokenId; i <= lastTokenId; i++) {
        debugger;
        let isExist = await nftContract.methods["_gnftExists"](i).call();
        if (!isExist) {
            ...
            await nftContract.methods["mintNFT"](groupName, i, weiVal).send({ from: account });
            break;
        }
    }
}
