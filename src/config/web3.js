...
export const getWalletTVL = async (provider, account, contractAddr) => {
  console.log("Factory Contract Called:");
  let web3 = new Web3(provider);
  ...
  // console.log("Factory [Wallet TVL]:", contract);
  let pLength = await factoryContract.methods["allPoolsLength"]().call();
  console.log("Pool Lengt:", pLength);
  let tvlBalance = 0;
  for (let i = 0; i < pLength; i++) {
    ...
    tvlBalance =
      tvlBalance +
      parseInt(await poolContract.methods["balanceOf"](account).call());
  }
  return tvlBalance;
};
// Get All Pools
export const getAllPools = async (provider, account, contractAddr) => {
  //   console.log("Get All Pools Called....");
  let web3 = new Web3(provider);
  const factoryABI = hedgeFactoryABI[0];
  const factoryContract = new web3.eth.Contract(factoryABI, contractAddr);
  ...
  let pLength = await factoryContract.methods["allPoolsLength"]().call();
  //   console.log("Pool Length:", pLength);
  let pools = [];
  for (let i = 0; i < pLength; i++) {
    let poolAddress = await factoryContract.methods["allPools"](i).call();
    poolContract.options.address = poolAddress;
    ...
    // console.log("Pool No.", i, "TS:", totalSupply, ":", poolContract);
    // --------
    console.log("Pool #", i, ":", poolContract);
    ...
    let userlp = ethers.utils.formatEther(
      await poolContract.methods["balanceOf"](account).call()
    );
    console.log(
      "contraaaaaaaaaaaaaaaaccccccccccccccct",
      weight,
      decimals,
      "sdfddf",
      poolTokenAndBalance,
    ...
    // let smbl = await poolContract.methods["name"]().call();
    // let _smbl = await poolContract.methods["_owner"]().call();
    // console.log("Pool T&B:", poolTokenAndBalance, "SBML:", smbl, "O:", _smbl);
    ...
    await axios
      ...
      .then((response) => {
        ...
        console.log("sbnmdf nmsdfmdsn ", a, "jksfdkjsdfkjdsfn", b);
        if (
          response?.data?.coins[Object.keys(response?.data?.coins)[0]]?.price >
            0.9 &&
          response?.data?.coins[Object.keys(response?.data?.coins)[0]]?.price <
          ...
          pools.push({
            address: poolAddress,
            totalSupply: (lp * userlp).toFixed(2),
          });
          console.log("heyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyy", lp * userlp);
        } else if (
          ...
          console.log("hiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiii", lp * userlp);
        } else {
          let lp =
            (a * (poolTokenAndBalance?.balances[0] / 10 ** 18) +
              poolTokenAndBalance?.balances[1] / 10 ** 18) /
        ...
        // pools.push({
        //   address: poolAddress,
        //   totalSupply: _tokensPrice.toFixed(2),
        // });
        // return (
    ...
    //     )
    //     .then((response) => {
    //       console.log("apiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiii", response);
    //       // Return Price in $
    //       // console.log(
    //       //   "To Wei:",
    //       //   parseInt(
...
export const getHoldingInLP = async (provider, account, contractAddr) => {
  let web3 = new Web3(provider);
  const factoryABI = hedgeFactoryABI[0];
  const factoryContract = new web3.eth.Contract(factoryABI, contractAddr);
  const pAbi = poolABI[0];
  ...
  for (let i = 0; i < pLength; i++) {
    ...
    //-------
    let totalSupply = await poolContract.methods["totalSupply"]().call();
    let circulatingPrice = await axios.get(
      "https://api.kava.io/vesting/circulatingsupplyswp"
    );
    console.log(
      "P#",
      i,
      ...
      totalSupply,
      "circulatingPrice:",
      circulatingPrice?.data
    );
    // console.log(
    //   "Pool No.",
    //   i,
    //   "TS:",
    //   parseFloat(utils.formatEther(totalSupply)),
    //   ":",
    //   poolContract
    // );
    //=====
    ...
    LPHolding.push({
      ...
      balance:
        (1 * 14916850) /
        parseFloat(utils.formatEther(circulatingPrice?.data.toString())),
    }); // place tvlBalance
  }
  return LPHolding;
};
...
export const approveToken = async (
  ...
  try {
    await token_contract.methods["approve"](contractAddr, weiVal).send({
      from: account,
    });
  } catch (e) {
    console.log(e.message);
  }
  const result = await tokenApproval(
    account,
    provider,
    tokenAddr,
    contractAddr
...
export const swapTokens = async (
  ...
  else wei_amount = await toWeiVal(provider, inTokenAddr, amount);

  if (outTokenAddr === "0x0000000000000000000000000000000000000000")
    wei_limit = web3.utils.toWei(limit.toString());
  else wei_limit = await toWeiVal(provider, outTokenAddr, limit);
...
export const initAddPool = async (
  ...
  else token1_wei_val = await toWeiVal(provider, token1Addr, amountA);
  if (token2Addr === "0x0000000000000000000000000000000000000000")
    token2_wei_val = web3.utils.toWei(amountB.toString());
  else token2_wei_val = await toWeiVal(provider, token2Addr, amountB);
...
export const removePool = async (
  ...
  const abi = routerABI[0];
  let web3 = new Web3(provider);
  const totalAmount = await web3.utils.toWei(
    (Math.floor(amount * Math.pow(10, 16)) / Math.pow(10, 16)).toFixed(16)
  );
  ...
  const limit1 =
    token1Amount > 0.00001 && token2Amount > 0.00001
      ? await toWeiVal(
          provider,
          token1Addr,
          (token1Amount * (1 - 0.3 - slippage)).toString()
        )
      : "0";
  const limit2 =
    token1Amount > 0.00001 && token2Amount > 0.00001
      ? await toWeiVal(
          provider,
          token2Addr,
          (token2Amount * (1 - 0.3 - slippage)).toString()
        )
      : "0";
...
const toWeiVal = async (provider, tokenAddr, val) => {
  ...
  let value = Number(
    (Math.floor(val * Math.pow(10, decimal)) / Math.pow(10, decimal)).toFixed(
      decimal
    )
  );
  try {
    let tval = web3.utils
      .toBN(toLongNum(value * 10 ** decimal).toFixed(0))
      .toString();
    return tval;
  } catch (error) {
...
export const calcOutput = async (
  ...
  try {
    if (middleTokens.length === 1) {
      ...
      const poolDataA = await getPoolData(provider, poolAddressA);
      ...
      const poolDataB = await getPoolData(provider, poolAddressB);
      ...
      const poolDataC = await getPoolData(provider, poolAddressC);
